...
class auth
{
    ...
    {
        \NI_security::unauthorized('login', true, '/org');
        \NI_security::unauthorized('role', 'org', '/org');
        $path = ['AdminApp','Login'];
        $static = [
            'css_arr'=>['Login'],
            'footer_js_arr'=>['Login'],
    ...
    public static function loginfunc()
    {
        ...
        $user = \model\org::check($RequestData);
        if (!empty($user)) {
            ...
            \NI_redirect::with('/org', 'primary', 'تم تسجيل الدخول بنجاح');
        } else {
